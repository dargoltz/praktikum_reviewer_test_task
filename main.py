...
# 3. Условие (today - record.date).days < 7 исключение день ровно неделю назад - ошибка
...
# рекомендую сделать отдельный класс-перечисление для типов валют (из модуля enum), но это необязательно
# 5. currency_type определяется неверно -
# он дублирует currency, когда должен переводить тип валюты в корректный для вывода вид
...
class CashCalculator(Calculator):
    ...
    def _convert_cash(self, amount: float, currency: str) -> float:
        # Используем словарь-диспетчер - один из альтернативных вариантов конструкции if-elif-else (необязательно)
        converted_amount = {
            "rub": amount,
